...
class StorageApi(private val context:Context,private val document:(String,String,Int)->DocumentFile){
    ...
    @Synchronized fun upload(r:HttpRequest,device:String):HttpReply?{
        ...
        require(saved.getLong("offset")==m.getLong("size"));if(!partial.exists()&&m.getLong("size")==0L)partial.createNewFile();val merkle=MerkleAccumulator();partial.inputStream().use{input->val buffer=ByteArray(chunkSize);while(true){val n=readChunk(input,buffer);if(n==0)break;merkle.addChunk(buffer.copyOf(n))}};if(!MessageDigest.isEqual(merkle.root(),Base64.getDecoder().decode(m.getString("merkleRoot")))){RandomAccessFile(partial,"rw").use{it.setLength(0);it.fd.sync()};saved.put("offset",0);write(state,saved);throw IOException("무결성 오류 · 다시 전송하세요")};var name=request.getString("name");var index=1;while(parent.findFile(name)!=null){val original=File(request.getString("name"));name="${original.nameWithoutExtension} (${index++}).${original.extension}"};val target=parent.createFile("application/octet-stream",name)?:throw IOException();try{DocumentCopies.publishNew(context,partial,target.uri){document(request.getString("shareId"),request.getString("path"),8)};saved.put("completed",true).put("uri",target.uri.toString());write(state,saved);partial.delete()}catch(e:Exception){target.delete();throw e};return HttpReply.text("{}")
    }
    private fun completedValid(saved:JSONObject):Boolean = try {
        val m=saved.getJSONObject("request").getJSONObject("manifest")
        val merkle=MerkleAccumulator();var length=0L
        context.contentResolver.openInputStream(android.net.Uri.parse(saved.getString("uri")))!!.use{input->
            val buffer=ByteArray(chunkSize);while(true){val n=readChunk(input,buffer);if(n==0)break;length+=n;merkle.addChunk(buffer.copyOf(n))}
        }
        length==m.getLong("size")&&MessageDigest.isEqual(merkle.root(),Base64.getDecoder().decode(m.getString("merkleRoot")))

...
class MeshEngine(val context: Context): AutoCloseable {
    ...
    private val requestingDevice = ThreadLocal<String?>()
    private val storageApi = StorageApi(context) {share,path,permission->document(share,path,permission)}
    ...
    var paused: Boolean
        ...
        set(value) { settings.edit().putBoolean("paused",value).commit() }
    init {
        val remembered=JSONArray(settings.getString("knownPeers","[]")!!)
        for(i in 0 until remembered.length()){
            val p=remembered.getJSONObject(i);val id=p.getString("id")
            if(trust.has(id))peers[id]=Peer(id,p.getString("name"),p.getString("address"),p.getInt("port"),false)
        }
    }
    ...
    @Synchronized private fun complete(p:PairSession){if(p.localAccepted&&p.remoteAccepted&&!p.rejected&&System.currentTimeMillis()<p.expires){trust.put(p.peer.id,p.remote.getString("fingerprint"));peers[p.peer.id]=p.peer;settings.edit().putString("trust",trust.toString()).commit();rememberPeers();status="${p.peer.name} 페어링 완료"}}
    @Synchronized private fun rememberPeers(){val values=JSONArray();peers.values.filter{trust.has(it.id)}.forEach{values.put(JSONObject().put("id",it.id).put("name",it.name).put("address",it.address).put("port",it.port))};settings.edit().putString("knownPeers",values.toString()).apply()}
    fun connection(peer:Peer,path:String,method:String="GET",body:ByteArray?=null,secure:Boolean=true):HttpsURLConnection {
        ...
        val endpoint=peers[peer.id]?:peer
    ...
    fun access(device:String,share:String,path:String,permission:Int):DocumentFile {if(!trusted(device))throw SecurityException("먼저 페어링하세요");return document(share,path,permission,device)}
    ...
    private fun relay(r:HttpRequest):HttpReply {val token=r.path.split('/').getOrNull(2)?:return HttpReply.text("",410);val s=streams[token]?:return HttpReply.text("",410);if(System.currentTimeMillis()-s.third>900000){streams.remove(token);return HttpReply.text("",410)};streams[token]=s.copy(third=System.currentTimeMillis());val peer=peers[s.first]?:return HttpReply.text("",404);val c=connection(peer,s.second,r.method);listOf("Range","If-Range").forEach{k->r.headers[k.lowercase()]?.let{c.setRequestProperty(k,it)}};val code=c.responseCode;val headers=mutableMapOf<String,String>();listOf("Content-Range","Accept-Ranges","ETag","Last-Modified").forEach{k->c.getHeaderField(k)?.let{headers[k]=it}};val input=if(code in 200..299)c.inputStream else c.errorStream;return HttpReply(code,c.contentType?:"application/octet-stream",maxOf(0,c.contentLengthLong),input?.let{object:FilterInputStream(it){override fun close(){super.close();c.disconnect()};override fun read(b:ByteArray,o:Int,l:Int):Int{streams[token]=s.copy(third=System.currentTimeMillis());return super.read(b,o,l)}}},headers)}
    ...
    @Synchronized fun setDevicePermissions(id:String,device:String,value:Int?){require(value==null||value in 0..15);for(i in 0 until shares.length()){val s=shares.getJSONObject(i);if(s.getString("id")==id){val overrides=s.optJSONObject("deviceOverrides")?:JSONObject();if(value==null)overrides.remove(device)else overrides.put(device,value);s.put("deviceOverrides",overrides)}};settings.edit().putString("shares",shares.toString()).commit()}
    ...
    @Synchronized private fun document(share:String,path:String,permission:Int,device:String?=requestingDevice.get()):DocumentFile {if(paused)throw SecurityException();val s=(0 until shares.length()).map{shares.getJSONObject(it)}.first{it.getString("id")==share};ShareAccess.require(s,device,permission);var doc=DocumentFile.fromTreeUri(context,Uri.parse(s.getString("uri")))?:throw IOException();for(part in PairingProtocol.safeParts(path)){if(part.startsWith('.'))throw SecurityException();doc=doc.findFile(part)?:throw FileNotFoundException()};return doc}
    private fun serve(r:HttpRequest):HttpReply {
        val fp=r.peer?.let{DeviceSecurity.fingerprint(it)}
        requestingDevice.set(synchronized(this){trust.keys().asSequence().firstOrNull{trust.getString(it)==fp}})
        try{return serveRequest(r)}finally{requestingDevice.remove()}
    }
    private fun serveRequest(r:HttpRequest):HttpReply {
        ...
        if(r.path=="/v1/secure/storage/shares"){if(paused)throw SecurityException();val array=JSONArray();synchronized(this){for(i in 0 until shares.length()){val s=shares.getJSONObject(i);val access=ShareAccess.permissions(s,requestingDevice.get());if(access and 1 != 0)array.put(JSONObject().put("id",s.getString("id")).put("name",s.getString("name")).put("permissions",access))}};return HttpReply.text(array.toString())}

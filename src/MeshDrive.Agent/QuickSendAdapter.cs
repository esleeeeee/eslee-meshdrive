...
public static class QuickSendAdapter
{
    ...
    {
        if (manifest.Size < 0 || manifest.ChunkSize != ChunkSize || manifest.LeafCount != (manifest.Size + ChunkSize - 1) / ChunkSize || Convert.FromBase64String(manifest.MerkleRoot).Length != 32)
